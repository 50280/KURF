...
# Computing additional mixed models for second revision. 
library(nlme)

# Pivoting the data to be in required format. 
long_data <- port_preprocessed_data %>%
  pivot_longer(
    cols = starts_with("first_") | starts_with("last_treatment_"),  
    names_to = c(".value", "treatment_type"),  
    names_pattern = "(first|last_treatment)_(.*)" ) %>%
  rename(baseline_score = first,  
         final_score = last_treatment)

# Implementing the mixed model
mixed_model <- lme(final_score ~ baseline_score + 
                   retrospectively_treatment_helpful + retrospectively_reported_improvement + 
                   demographics_gender + demographics_age_at_screening, 
